...
const componentName = changeCase.pascalCase(process.argv[3]);
...
if (objectValues(componentTypes).indexOf(componentType) === -1) {
    process.stdout.write(`Third argument should be empty (default is "${componentTypes.PURE_REACT}") or one of ${objectValues(componentTypes).map(x => `"${x}"`).join(', ')}\n`);
    process.exit(1);
}
...
function processFile(absPath) {
    const relativeFilePath = absPath.replace(componentSourcePath, '').replace(originalComponentName, componentName);
...
function replaceFileContent(content) {
    return content.replace(new RegExp(originalComponentName, 'g'), componentName).replace(new RegExp(changeCase.paramCase(originalComponentName), 'g'), changeCase.paramCase(componentName));
}
